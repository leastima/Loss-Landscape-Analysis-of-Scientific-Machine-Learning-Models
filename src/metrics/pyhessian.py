...
class hessian():
  ...
  def trace(self, num_samples=100, tol=1e-3):
    trace_estimates = []
    prev_avg = None
    ...
    for i in range(num_samples):
        v = [2 * torch.randint_like(p, high=2, device=self.device) - 1 for p in self.params]
        Hv = hessian_vector_product(self.gradsH, self.params, v)
        trace_i = group_product(v, Hv).cpu().item()
        trace_estimates.append(trace_i)

        current_avg = sum(trace_estimates) / len(trace_estimates)

        if prev_avg is not None:
            rel_change = abs(current_avg - prev_avg) / (abs(prev_avg) + 1e-6)
            if rel_change < tol:
                break
