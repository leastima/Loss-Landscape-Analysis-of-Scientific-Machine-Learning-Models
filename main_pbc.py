...
print(X_star_noinitial_noboundary.shape)
...
metrics['Error_u_rel'] = error_u_relative
metrics['Error_u_abs'] = error_u_abs
metrics['Error_u_linf'] = error_u_linf
...
# for training loss
loss_x_u = torch.tensor(X_u_train[:, 0:1], requires_grad=True).float().to(device)
loss_t_u = torch.tensor(X_u_train[:, 1:2], requires_grad=True).float().to(device)
u = torch.tensor(u_train, requires_grad=True).float().to(device)

loss_x_f = torch.tensor(X_f_train[:, 0:1], requires_grad=True).float().to(device)
loss_t_f = torch.tensor(X_f_train[:, 1:2], requires_grad=True).float().to(device)

loss_x_bc_lb = torch.tensor(bc_lb[:, 0:1], requires_grad=True).float().to(device)
loss_t_bc_lb = torch.tensor(bc_lb[:, 1:2], requires_grad=True).float().to(device)
loss_x_bc_ub = torch.tensor(bc_ub[:, 0:1], requires_grad=True).float().to(device)
loss_t_bc_ub = torch.tensor(bc_ub[:, 1:2], requires_grad=True).float().to(device)

if torch.is_grad_enabled():
    model.optimizer.zero_grad()
    
loss_u_pred = model.net_u(loss_x_u, loss_t_u)
loss_u_pred_lb = model.net_u(loss_x_bc_lb, loss_t_bc_lb)
loss_u_pred_ub = model.net_u(loss_x_bc_ub, loss_t_bc_ub)
if nu != 0:
    loss_u_pred_lb_x, loss_u_pred_ub_x = model.net_b_derivatives(loss_u_pred_lb, loss_u_pred_ub, loss_x_bc_lb, loss_x_bc_ub)

loss_f_pred = model.net_f(loss_x_f, loss_t_f)


loss_u = torch.mean((u - loss_u_pred) ** 2)
loss_b = torch.mean((loss_u_pred_lb - loss_u_pred_ub) ** 2)
if nu != 0:
    loss_b += torch.mean((loss_u_pred_lb_x - loss_u_pred_ub_x) ** 2)
loss_f = torch.mean(loss_f_pred ** 2)

loss = loss_u + loss_b + loss_f
metrics['training_loss_u'] = loss_u.item()
metrics['training_loss_b'] = loss_b.item()
metrics['training_loss_f'] = loss_f.item()
...
hessian_x_u = torch.tensor(X_u_train[:, 0:1], requires_grad=True).float().to(device)
hessian_t_u = torch.tensor(X_u_train[:, 1:2], requires_grad=True).float().to(device)
hessian_u = torch.tensor(u_train, requires_grad=True).float().to(device)

hessian_x_f = torch.tensor(X_f_train[:, 0:1], requires_grad=True).float().to(device)
hessian_t_f = torch.tensor(X_f_train[:, 1:2], requires_grad=True).float().to(device)

hessian_x_bc_lb = torch.tensor(bc_lb[:, 0:1], requires_grad=True).float().to(device)
hessian_t_bc_lb = torch.tensor(bc_lb[:, 1:2], requires_grad=True).float().to(device)
hessian_x_bc_ub = torch.tensor(bc_ub[:, 0:1], requires_grad=True).float().to(device)
hessian_t_bc_ub = torch.tensor(bc_ub[:, 1:2], requires_grad=True).float().to(device)
...
if torch.is_grad_enabled():
    model.optimizer.zero_grad()
hessian_comp = hessian_pinn(nu, model_init, criterion, data=(x, t), u_data=(hessian_x_u, hessian_t_u, hessian_u), f_data=(hessian_x_f, hessian_t_f), b_data=(hessian_x_bc_lb, hessian_t_bc_lb, hessian_x_bc_ub, hessian_t_bc_ub), L=args.L, cuda=True)
top_eigenvalues, top_eigenvector = hessian_comp.eigenvalues()
trace = np.mean(hessian_comp.trace())
print("Trace: ", trace)
print("Top eigenvalue: ", top_eigenvalues[0])
metrics['hessian_trace'] = trace
metrics['top_eigenvalues'] = top_eigenvalues[0]

if not os.path.exists(args.save_path):
    os.makedirs(args.save_path)
np.save(args.save_path + '/base_metrics.npy', metrics)
